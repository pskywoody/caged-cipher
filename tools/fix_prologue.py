...
# New prologue sequence per user's requirement
new_prologue = [
    {
        'type': 'title',
        'text': '第一章',
    ...
    {
        'speaker': '旁白',
        'text': '你推开尘封三十年的档案馆大门，空气中弥漫着旧纸张的气息。一封匿名信带你来到这里——信上只有一行字："档案编号K-734。如果你能解开这一局，就能找到我。"',
        'isNarration': True,
        'bg': 'assets/images/backgrounds/bg_scene1_single_door_v2.jpg',
        'sfx': 'playDoorOpen',
        'item': 'file-k734'
...
print()
print('Sequence:')
for i, line in enumerate(new_prologue):
    ...
    item = line.get('item', '')
    typ = line.get('type', '')
    print('%d: [%s] %s | bg=%s sfx=%s item=%s | %s' % (
        i, typ or 'dlg', speaker or 'narrator',
        bg.split('/')[-1] if bg else '-', sfx or '-', item or '-', text))
